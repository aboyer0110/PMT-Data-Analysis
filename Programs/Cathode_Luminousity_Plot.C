...
void Cathode_Luminousity_Plot() {

char L, A; //characters to be read in from the data file
double Sk, Sp, Idb1, Idb2, Skb, Gain, UniPi_Gain, extra, SerialNumb, Product; //double variables, to be read mostly as dummy variables
...
TH1F *h1 = new TH1F("Cathode Luminous Sensitivity","Cathode Luminous Sensitivity", 25, 40, 150); //instantiate a histogram for the RICH 1 H12700s
TH1F *h2 = new TH1F("Cathode Luminous Sensitivity","Cathode Luminous Sensitivity", 25, 40, 150); //instantiate a histogram for the RICH 2 H12700s
TH1F *h3 = new TH1F("Cathode Luminous Sensitivity","Cathode Luminous Sensitivity", 25, 40, 150); //instantiate a histogram for the RICH 1 H8500s
gStyle->SetOptStat(1000000001); //option to disable the "entries" tracker in the statistics box, as they would be inaccurately displayed
...
h1->Fill(Sk); //fill the RICH 1 H12700 histo with the cathode luminousity
...
h2->Fill(Sk); //fill the RICH 2 H12700 histo with cathode luminousity
...
h3->Fill(Sk); //fill the RICH 1 H8500 histo with cathode luminousity
...
h1->GetXaxis()->SetTitle("Cathode Luminous Sensitivty (micro-amps/lm"); //title the RICH 1 H12700s histo x-axis
...
h1->SetMaximum(70); //set the maximum for the RICH 1 H12700 histo
h2->SetMaximum(70); //set the maximum for the RICH 2 H12700 histo
h3->SetMaximum(70); //set the maximum for the RICH 1 H8500 histo
...
legend->AddEntry(h1,"RICH 1 & 2 H12700s (Avg: 90.02)","l");
legend->AddEntry(h2,"RICH 2 H12700s (Avg: 82.04)","l");
legend->AddEntry(h3,"RICH 1 H8500s (Avg: 60.22)","l");
legend->Draw();
c2->Print("Cathode Luminous Sensitivity Distribution.pdf"); //print the canvas to a pdf
